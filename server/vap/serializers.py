...
from .models import Team, Coach, TeamMember, Deliverable, Milestone, Question, TeamDeliverable, Answer, Comment, CommentAnswer
...
class CommentSerializer(serializers.ModelSerializer):
    comment_answers = CommentAnswerSerializer(many=True)
    class Meta:
        model = Comment
        fields = '__all__'

    def create(self, validated_data):
        comment_answers_data = validated_data.pop('comment_answers')
        comment = Comment.objects.create(**validated_data)
        for comment_answers_data in comment_answers_data:
            CommentAnswer.objects.create(album=comment, **comment_answer_data)
        return comment
